...
func newProxyAwareWebsocketDialer(cfg *config.Config, auth *cliproxyauth.Auth) *websocket.Dialer {
	// Use utls for TLS connections to mimic Chrome fingerprint.
	// TLSClientConfig is nil so gorilla/websocket doesn't do its own TLS handshake.
	// Instead, NetDialContext wraps the TCP connection with utls before returning it.
	dialer := &websocket.Dialer{
		Proxy:             http.ProxyFromEnvironment,
		HandshakeTimeout:  codexResponsesWebsocketHandshakeTO,
		EnableCompression: true,
		TLSClientConfig:   nil, // utls handles TLS in NetDialContext
	}

	// NetDialContext with utls TLS wrapping for wss:// connections
	dialer.NetDialContext = func(ctx context.Context, network, addr string) (net.Conn, error) {
		...
		// For TLS connections (wss://), wrap with utls Chrome fingerprint
		// gorilla/websocket will detect this is already a TLS conn and skip its own handshake
		host, _, _ := net.SplitHostPort(addr)
		if host == "" {
			host = addr
		}
		return WrapConnWithUTLS(tcpConn, host, defaultChromeVersion)
	}
...
func applyCodexWebsocketHeaders(ctx context.Context, headers http.Header, auth *cliproxyauth.Auth, token string, cfg *config.Config) http.Header {
	...
	// Browser-like headers for anti-detection
